...
def get_copy_dataset_type(dataset_type: str) -> str:
    ...
    elif dataset_type in ['FewShotNEUDataset', 'FewShotNEUDefaultDataset']:
        copy_dataset_type = 'FewShotNEUCopyDataset'
    else:
...
def build_dataset(cfg: ConfigDict,
    ...
    elif cfg['type'] == 'MultiStageBaseDataset':
        novel_dataset = build_dataset(cfg["dataset"],default_args)
        if cfg.get('base_dataset', None) is not None: 
            dataset_list = []      
            base_cfg = copy.deepcopy(cfg["base_dataset"])
            oversample_times = base_cfg.pop("oversample_times",None)
            split,seed,shot = base_cfg["ann_cfg"][0]["setting"].split("_")
            seed=int(seed.strip("SEED"))
            for i in range(oversample_times):
                if seed+i==10:
                    seed_index = 10 
                else:
                    seed_index = (seed+i)%10                    
                base_cfg["ann_cfg"][0]["setting"]=split+f"_SEED{seed_index}_"+shot
                base_dataset = build_dataset(base_cfg,default_args)
                all_dataset=MultiStageBaseDataset(novel_dataset,base_dataset)
                
                dataset_list.append(all_dataset)
                # save dataset for the reproducibility
                if rank == 0 and save_dataset:
                    save_dataset_path = osp.join(work_dir, f'{timestamp}_saved_data_phase_{i}.json')
                    print(save_dataset_path)
                    if hasattr(all_dataset, 'save_data_infos'):
                        all_dataset.save_data_infos(save_dataset_path)
                    else:
                        raise AttributeError(
                            f'`save_data_infos` is not implemented in {type(all_dataset)}.')
        return dataset_list
    else:
    ...
    if isinstance(dataset, QueryAwareDataset):
        ...
        for data_ in dataset:
            ...
            data_loader_ = DataLoader(
                ...
                drop_last=True,
                **kwargs)
            data_loader.append(data_loader_)
    else:
        data_loader = DataLoader(
            dataset,
            batch_size=batch_size,
