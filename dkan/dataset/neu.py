...
# intra-class defects existing large differences in appearance, 
# for instance, the scratches (the last column) may be horizontal scratch, vertical scratch, and slanting scratch, etc. 
# Meanwhile the inter-class defects have similar aspects, e.g.,  crazing, pitted surface and rolled-in scale,.
...
class FewShotNEUDataset(BaseFewShotDataset):
    ...
    def load_annotations_xml(
        ...
        for img_id in range(len(img_names)):
            ...
            xml_path = osp.join(self.data_root,'annotations',f'{filename}.xml')
            ...
            if size is not None:
                ...
                img_path = osp.join(self.data_root,'images', '{}.jpg'.format(filename))
                img = mmcv.imread(img_path)
                width, height = img.size
            ann_info = self._get_xml_ann_info(filename, classes)
            data_infos.append(
                dict(
                    id=img_id,
...
@DATASETS.register_module()
class FewShotNEUCopyDataset(FewShotNEUDataset):
...
@DATASETS.register_module()
class FewShotNEUDefaultDataset(FewShotNEUDataset):
    ...
    voc_benchmark ={
        f'SPLIT{split}_SEED{seed}_{shot}SHOT': [
            dict(
                type='ann_file',
                ann_file=f'fewshot_split/seed{seed}/box_{shot}shot_{class_name}_train.txt',
                ann_classes=[class_name])
            for class_name in NEU_SPLIT[f'ALL_CLASSES_SPLIT{split}']
        ]
        for shot in [5,10,30] for seed in range(1,11) for split in [1,2,3]
    }
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        return super(FewShotNEUDataset, self).ann_cfg_parser(new_ann_cfg)


@DATASETS.register_module()
class FewShotNEUBaselineDataset(FewShotNEUDataset):
    """Dataset with some pre-defined VOC annotation paths.

    :obj:`FewShotNEUBaselineDataset` provides pre-defined annotation files
    to ensure the reproducibility. The pre-defined annotation files provide
    fixed training data to avoid random sampling. The usage of `ann_cfg' is
    different from :obj:`FewShotVOCDataset`. The `ann_cfg' should contain
    two filed: `method` and `setting`.
    ...
    def ann_cfg_parser(self, ann_cfg: List[Dict]) -> List[Dict]:
        ...
        for ann_cfg_ in ann_cfg:
            ...
            new_ann_cfg += default_ann_cfg
        return super(FewShotNEUBaselineDataset, self).ann_cfg_parser(new_ann_cfg)